...
from xunfei import  super_smart_tts as ss
import json
import sys
import os
...
cdata = 'data/'+con[2]
######################配置###################################
client = OpenAI(api_key=con[0], base_url="https://api.siliconflow.cn/v1") #输入api_key即可
...
def Txt_Create():   #创建数据文件
    if os.path.isfile(filename):  #存在则跳过
        ...
        txt_file = open(filename, 'w')  #没有则创建
...
def run_ai(q):
    ...
    if(stream):	
        for chunk in response:     
            strings=str(chunk.choices[0].delta.content)  #转化格式
            strings=strings.replace("None", "");   #修复deepseekR1异常文本
            m_answer=m_answer+strings  #连续流式对话数据
